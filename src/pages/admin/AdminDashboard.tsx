...
const AdminDashboard = () => {
  ...
  }>({ revenue: [], userTrend: [], activityDistribution: [] });
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      setStats({
        ...
        userActivityTrend: 0, // Will be updated below
      });
      ...
      const userTrendChartData = Object.entries(userTrendAcc).map(([date, count]) => ({ date, count })).sort((a: any, b: any) => new Date(a.date).getTime() - new Date(b.date).getTime());
        ...
        <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-10 pb-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <StatWidget
              ...
              icon={DollarSign}
              color="green"
              subValue="Commissions only"
              trend={analyticsData.revenue.map(d => d.amount)}
            />
            <StatWidget
            ...
            <div className="p-12 space-y-10">
              <div className="flex items-center justify-between">
                ...
                <div className={cn("px-4 py-2 rounded-2xl border font-black uppercase text-[10px] tracking-widest", selectedUser?.is_active ? "bg-emerald-50 text-emerald-600 border-emerald-100" : "bg-rose-50 text-rose-600 border-rose-100")}>
                  {selectedUser?.is_active ? 'Active User' : 'Suspended'}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div className="space-y-6">
                  ...
                  <div className="space-y-4 max-h-[300px] overflow-y-auto pr-4 scrollbar-hide">
                    ...
                    {userBookings.length === 0 && <p className="text-center py-10 text-[10px] font-black uppercase text-slate-300 tracking-widest">No Activity History</p>}
                  </div>
                </div>
